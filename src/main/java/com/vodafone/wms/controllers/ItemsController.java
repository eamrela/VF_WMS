...
import com.vodafone.wms.beans.ItemsFacade;
...
public class ItemsController implements Serializable {
    ...
    @EJB
    private com.vodafone.wms.beans.ItemsFacade ejbFacade;
    ...
    public Items getItems(java.lang.String id) {
    ...
    @FacesConverter("ItemsControllerConverter")
    ...
    public List<Items> searchItems(String query) {
        ...
        for (Items item : items) {
            if(item.getPartNumber().toLowerCase().startsWith(query.toLowerCase())){ // bom
                results.add(item);
            }
            if(item.getDescription()!=null){
            if(item.getDescription().toLowerCase().startsWith(query.toLowerCase())){ // description
                results.add(item);
            }
            }
            if(item.getMaterialNumber()!=null){
            if(item.getMaterialNumber().toLowerCase().startsWith(query.toLowerCase())){ // description
                results.add(item);
            }
            }
        }
